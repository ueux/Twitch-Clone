...
export async function POST(req: Request) {
  ...
  if (!WEBHOOK_SECRET) {
    throw new Error(
      `Please add CLERK_WEBHOOK_SECRET from Clerk Dashboard to .enx or .env.local`
    );
  }
  ...
  if (eventType === "user.deleted") {
    ...
    if (eventType === "user.updated") {
        const currentUser = await db.user.findUnique({ where: { externalUserId: payload.data.id } })
